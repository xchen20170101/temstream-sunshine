...
const std::filesystem::path PRIVATE_KEY_FILE = platf::appdata() / std::filesystem::path(CA_DIR) / "cakey.pem";
const std::filesystem::path CERTIFICATE_FILE = platf::appdata() / std::filesystem::path(CA_DIR) / "cacert.pem";
...
#undef _CONVERT_2_ARG_
  ...
  void apply_config(std::unordered_map<std::string, std::string> &&vars) {
    ...
    bool_f(vars, "enable_web_ui", sunshine.enable_web_ui);
