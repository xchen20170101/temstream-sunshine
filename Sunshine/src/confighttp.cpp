...
namespace confighttp {
  ...
  /**
   * @brief Get backend connection status (V4 Phase 2: device_id, current_pin, connected).
   * @param response The HTTP response object.
   * @param request The HTTP request object.
   *
   * @api_examples{/api/backend/status| GET| null}
   */
  void getBackendStatus(resp_https_t response, req_https_t request) {
    if (!authenticate(response, request)) {
      return;
    }
    ...
    nlohmann::json output_tree;
    output_tree["status"] = true;
    output_tree["device_id"] = status.device_id;
    output_tree["current_pin"] = status.current_pin;
    output_tree["connected"] = status.connected;
    send_response(response, output_tree);
  }
  ...
  void savePin(resp_https_t response, req_https_t request) {
    ...
    try {
      ...
      // Check LAN mode password if enabled
      if (nvhttp::getLanFixedPin()) {
        const std::string expected_password = nvhttp::getLanPassword();
        if (!expected_password.empty()) {
          const std::string client_password = input_tree.value("password", "");
          if (client_password != expected_password) {
            BOOST_LOG(warning) << "LAN mode password mismatch, expected: " << expected_password;
            output_tree["status"] = false;
            output_tree["error"] = "Invalid LAN password";
            send_response(response, output_tree);
            return;
          }
          BOOST_LOG(info) << "LAN mode password verified successfully";
        }
      }
  ...
  void start() {
    ...
    server.resource["^/api/backend/status$"]["GET"] = getBackendStatus;
