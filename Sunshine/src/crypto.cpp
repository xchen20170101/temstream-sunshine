...
namespace crypto {
  ...
  aes_t gen_aes_key(const std::array<uint8_t, 16> &salt, const std::string_view &pin, const std::string_view &password) {
    aes_t key(16);

    std::string salt_pin_pass;
    salt_pin_pass.reserve(salt.size() + pin.size() + password.size());

    salt_pin_pass.insert(std::end(salt_pin_pass), std::begin(salt), std::end(salt));
    salt_pin_pass.insert(std::end(salt_pin_pass), std::begin(pin), std::end(pin));
    if (!password.empty()) {
      salt_pin_pass.insert(std::end(salt_pin_pass), std::begin(password), std::end(password));
    }

    auto hsh = hash(salt_pin_pass);
