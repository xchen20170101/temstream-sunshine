...
namespace http {
  ...
  int init() {
    ...
    if (clean_slate) {
      unique_id = uuid_util::generate_uuid_string();
    ...
    if (!user_creds_exist(config::sunshine.credentials_file)) {
      // 创建默认凭证：用户名 sunshine，密码 sunshine
      save_user_creds(config::sunshine.credentials_file, "sunshine", "sunshine@asd123", true);
      reload_user_creds(config::sunshine.credentials_file);
      BOOST_LOG(info) << "Default credentials created: username=sunshine, password=sunshine@asd123";
