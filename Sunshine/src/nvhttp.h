...
namespace nvhttp {
   ...
   * @param uniqueID The unique ID of the client session (for multi-device support).
   * @return `true` if the pin is correct, `false` otherwise.
   * @examples
   * bool pin_status = nvhttp::pin("1234", "laptop", "uniqueid123");
   * @examples_end
   */
  ...
  // 动态PIN码配对相关函数（sendPin 现在接受 uniqueID 参数以支持多设备配对）
  ...
  // V4 Phase 2: 供 backend_client 使用的函数
  extern const char *DEVICE_API_KEY;
  std::string getReportedIp();
}  // namespace nvhttp
