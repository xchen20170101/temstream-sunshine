...
namespace system_tray {
  ...
  void tray_config_cb([[maybe_unused]] struct tray_menu *item) {
    BOOST_LOG(info) << "Opening Config from system tray"sv;

    // Path to the Config executable (same directory as sunshine.exe)
    auto config_exe = platf::appdata().parent_path() / "sunshine_config.exe";

    boost::process::v1::environment _env = boost::this_process::environment();
    boost::filesystem::path work_dir;
    std::error_code ec;
    auto child = platf::run_command(false, false, config_exe.string(), work_dir, _env, nullptr, ec, nullptr);
    if (ec) {
      BOOST_LOG(warning) << "Couldn't open Config ["sv << config_exe.string() << "]: System: "sv << ec.message();
    } else {
      BOOST_LOG(info) << "Opened Config ["sv << config_exe.string() << "]"sv;
      child.detach();
    }
  }
