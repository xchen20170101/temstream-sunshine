...
rem Set explicit write permissions on config directory using localized SIDs.
rem Disable inheritance so we don't inherit the read-only ACL from Program Files,
rem then grant Users (non-admin accounts running sunshine_config.exe) full control.
rem Note: We must use the SIDs directly because "Users" and "Administrators" are localized.
icacls "%NEW_DIR%" /inheritance:r
icacls "%NEW_DIR%" /grant:r *S-1-5-32-545:(OI)(CI)F
icacls "%NEW_DIR%" /grant:r *S-1-5-32-544:(OI)(CI)F
icacls "%NEW_DIR%" /grant:r *S-1-5-18:(OI)(CI)F
