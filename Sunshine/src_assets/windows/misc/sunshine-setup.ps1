...
if ($Action -eq "install") {
    ...
    # 3.1. Set permissions on config directory after migration
    # Migrate-config.bat previously reset config ACL to Program Files read-only inheritance.
    # To fix that, we disable inheritance first, reset, then explicitly grant Users/Admins/SYSTEM
    # using /grant:r with localized SIDs (Everyone can be blocked by Program Files ACL integrity).
    $currentStep++
    Write-Progress `
        -Activity "Installing Sunshine" `
        -Status "Setting permissions on config directory" `
        -PercentComplete (($currentStep / $totalSteps) * 100)
    Write-LogMessage -Message "🔐 Setting permissions on config directory (after migration)" -Level "Step"
    ...
    try {
        ...
        Write-LogMessage -Message "Executing: icacls.exe `"$configDir`" /inheritance:r ..." -Level "Information"
        ...
        try {
            # Step 1: Disable inheritance from parent (e.g. Program Files).
            # Without this, /grant additions are blocked by the protected Program Files ACL,
            # and the directory inherits a read-only ACL that prevents non-admin users from writing.
            $icaclsProcess = Start-Process `
                -FilePath "icacls.exe" `
                -ArgumentList "`"$configDir`" /inheritance:r /c" `
                -Wait `
                -PassThru `
                -NoNewWindow `
                -RedirectStandardOutput $stdoutFile `
            ...
            # Step 2: Reset existing explicit permissions so the /grant:r below replaces cleanly.
            $icaclsProcess = Start-Process `
                -FilePath "icacls.exe" `
                -ArgumentList "`"$configDir`" /reset /t /c" `
                -Wait `
                -PassThru `
                -NoNewWindow `
                -RedirectStandardOutput $stdoutFile `
            ...
            # Step 3: Grant Users full control (replace mode). Use the localized SID
            # *S-1-5-32-545 because the well-known name "Users" can be localized and
            # "Everyone" can be blocked by the Program Files ACL integrity check.
            $icaclsProcess = Start-Process `
                -FilePath "icacls.exe" `
                -ArgumentList "`"$configDir`" /grant:r *S-1-5-32-545:(OI)(CI)F /t /c" `
                -Wait `
                -PassThru `
                -NoNewWindow `
                -RedirectStandardOutput $stdoutFile `
            ...
            # Step 4: Grant Administrators full control (replace mode). Use SID for localization.
            $icaclsProcess = Start-Process `
                -FilePath "icacls.exe" `
                -ArgumentList "`"$configDir`" /grant:r *S-1-5-32-544:(OI)(CI)F /t /c" `
                -Wait `
                -PassThru `
                -NoNewWindow `
                -RedirectStandardOutput $stdoutFile `
            ...
            # Step 5: Grant SYSTEM full control so the SunshineService can write to config.
            $icaclsProcess = Start-Process `
                -FilePath "icacls.exe" `
                -ArgumentList "`"$configDir`" /grant:r *S-1-5-18:(OI)(CI)F /t /c" `
                -Wait `
                -PassThru `
                -NoNewWindow `
                -RedirectStandardOutput $stdoutFile `
        ...
        Write-LogMessage -Message "  ⚠ Failed to set permissions: $($_.Exception.Message)" -Level "Warning"
    }
    Write-Information ""
