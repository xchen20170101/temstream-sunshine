...
public class ComputerManagerService extends Service {
    ...
    private volatile boolean discoveryReady;
    ...
    public class ComputerManagerBinder extends Binder {
        public void startPolling(ComputerManagerListener listener) {
            ...
            // 【已移除】不再启动mDNS自动发现，改为使用云端API获取设备列表
        ...
        public void waitForReady() {
            synchronized (discoveryServiceConnection) {
                ...
                try {
                    while (!discoveryReady) {
                        // Wait for either the discovery binder or a simulated ready state.
    ...
    public void onCreate() {
        // 【已移除】不再绑定到发现服务，改为使用云端API
        // bindService(new Intent(this, DiscoveryService.class),
        //         discoveryServiceConnection, Service.BIND_AUTO_CREATE);
        
        // 模拟discoveryBinder已准备好，避免waitForReady()等待
        synchronized (discoveryServiceConnection) {
            discoveryBinder = null; // 设置为null，表示不使用mDNS发现
            discoveryReady = true;
            discoveryServiceConnection.notifyAll();
        }
