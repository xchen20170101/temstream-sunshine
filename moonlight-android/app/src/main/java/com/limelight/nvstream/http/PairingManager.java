...
public class PairingManager {
    ...
    private static byte[] saltPin(byte[] salt, String pin, String password) throws UnsupportedEncodingException {
        byte[] saltedPin;
        if (password != null && !password.isEmpty()) {
            saltedPin = new byte[salt.length + pin.length() + password.length()];
            System.arraycopy(salt, 0, saltedPin, 0, salt.length);
            System.arraycopy(pin.getBytes("UTF-8"), 0, saltedPin, salt.length, pin.length());
            System.arraycopy(password.getBytes("UTF-8"), 0, saltedPin, salt.length + pin.length(), password.length());
        } else {
            saltedPin = new byte[salt.length + pin.length()];
            System.arraycopy(salt, 0, saltedPin, 0, salt.length);
            System.arraycopy(pin.getBytes("UTF-8"), 0, saltedPin, salt.length, pin.length());
        }
        return saltedPin;
    ...
    public PairState pair(String serverInfo, String pin, String password) throws IOException, XmlPullParserException {
        ...
        // Combine the salt and pin (and password for LAN mode), then create an AES key from them
        byte[] aesKey = generateAesKey(hashAlgo, saltPin(salt, pin, password));
