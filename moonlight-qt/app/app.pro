...
unix:!macx: {
    ...
    desktop.files = deploy/linux/com.temstream.TemStream.desktop
    ...
    appstream.files = deploy/linux/com.temstream.TemStream.appdata.xml
    appstream.path = $$PREFIX/$$DATADIR/metainfo/

    INSTALLS += target desktop icons appstream
}
win32 {
    RC_ICONS = temstream.ico
    QMAKE_TARGET_COMPANY = TemStream Project
    QMAKE_TARGET_DESCRIPTION = TemStream Game Streaming Client
    QMAKE_TARGET_PRODUCT = TemStream

    CONFIG -= embed_manifest_exe
    QMAKE_LFLAGS += /MANIFEST:embed /MANIFESTINPUT:$${PWD}/temstream.exe.manifest
}
macx {
    ...
    APP_BUNDLE_RESOURCES.files = temstream.icns
