...
ComputerManager::ComputerManager(QObject *parent)
    : QObject(parent),
      ...
      m_targetPin("0000"),
      m_DeviceList(),
      m_LanFixedPinMode(false)
{
    QSettings settings;

    // 从 conf.ini 加载局域网固定 PIN 模式配置
    reloadLanFixedPinMode();
...
void ComputerManager::startPolling()
{
    ...
    // Use the configured value from StreamingPreferences
    if (prefs.enableMdns) {
...
public:
    PendingPairingTask(ComputerManager* computerManager, NvComputer* computer, QString pin, QString password = "")
...
private:
    void run()
    {
        // In LAN fixed PIN mode, use the saved password for PIN key derivation
        // The password was already verified when connecting in connectToLanServer()
        if (m_ComputerManager->getLanFixedPinMode()) {
            QString lanPassword = m_ComputerManager->getLanPassword();
            if (!lanPassword.isEmpty() && m_Password.isEmpty()) {
                m_Password = lanPassword;
            }
        }

        NvPairingManager pairingManager(m_Computer);

        // V4: 如果设备有保存的 PIN，优先使用设备特定的 PIN
        QString pinToUse = m_Pin;
        if (!m_Computer->pin.isEmpty()) {
            pinToUse = m_Computer->pin;
            qDebug() << "Using device-specific PIN for pairing:" << m_Computer->name;
        }

        try {
           NvPairingManager::PairState result = pairingManager.pair(m_Computer->appVersion, pinToUse, m_Computer->serverCert, m_Password);
           switch (result)
           {
...
void ComputerManager::pairHost(NvComputer* computer, QString pin, QString password)
{
    // Punt to a worker thread to avoid stalling the
    // UI while waiting for pairing to complete
    PendingPairingTask* pairing = new PendingPairingTask(this, computer, pin, password);
...
void ComputerManager::addNewHostManually(QString address, QString customName, QString pin)
{
    ...
    if (url.isValid() && !url.host().isEmpty() && url.scheme() == "moonlight") {
        ...
        addNewHost(NvAddress(url.host(), url.port(DEFAULT_HTTP_PORT)), false, NvAddress(), customName, pin);
    }
    else {
...
void ComputerManager::setPin(QString pin)
{
    ComputerManager::m_targetPin = pin;
}

QString ComputerManager::getServerIp()
{
    // 检查配置文件是否存在
    QFileInfo fileInfo("./conf.ini");
    if (!fileInfo.exists()) {
        // 如果配置文件不存在，则创建一个新的配置文件并写入默认值
        QSettings settings("./conf.ini", QSettings::IniFormat);
        settings.setValue("serverIp", "127.0.0.1"); // 设置默认的服务器IP地址
    }

    // 读取配置文件中的服务器IP地址
    QSettings settings("./conf.ini", QSettings::IniFormat);
    return settings.value("serverIp").toString();
}

void ComputerManager::setServerIp(QString ip)
{
    QSettings settings("./conf.ini", QSettings::IniFormat);
    settings.setValue("serverIp", ip);
}

QString ComputerManager::getLanServerIp()
{
    // 读取配置文件中的局域网模式 Sunshine 服务端IP
    QSettings settings("./conf.ini", QSettings::IniFormat);
    return settings.value("lanServerIp", "127.0.0.1").toString();
}

void ComputerManager::setLanServerIp(QString ip)
{
    QSettings settings("./conf.ini", QSettings::IniFormat);
    settings.setValue("lanServerIp", ip);
}

QVariantMap ComputerManager::verifyLanPassword(QString serverIp, QString password)
{
    // 调用NvHTTP的静态方法验证密码
    return NvHTTP::verifyLanPassword(serverIp, password);
}

QString ComputerManager::getLanPassword()
{
    // 读取配置文件中的局域网模式密码
    QSettings settings("./conf.ini", QSettings::IniFormat);
    return settings.value("lanPassword", "").toString();
}
...
public:
    ...
    PendingAddTask(ComputerManager* computerManager, NvAddress address, NvAddress mdnsIpv6Address, bool mdns, QString customName = QString(), QString pin = QString())
        : m_ComputerManager(computerManager),
          m_Address(address),
          ...
          m_AboutToQuit(false)
    {
        ...
        qInfo() << "DEBUG PendingAddTask: constructed task" << m_TaskId;
...
private:
    ...
    QString fetchServerInfo(NvHTTP& http)
    {
        ...
        if (m_AboutToQuit) {
            qInfo() << "DEBUG fetchServerInfo: taskId" << m_TaskId << "m_AboutToQuit=true, returning empty";
            return QString();
        }

        try {
            qInfo() << "DEBUG fetchServerInfo: taskId" << m_TaskId << "calling getServerInfo()";
            ...
            try {
                ...
                qInfo() << "DEBUG fetchServerInfo: taskId" << m_TaskId << "getServerInfo returned, length:" << serverInfo.length();
            } catch (const QtNetworkReplyException& e) {
            ...
            qInfo() << "DEBUG fetchServerInfo: taskId" << m_TaskId << "caught exception, emitting false";
            if (!m_Mdns) {
                StreamingPreferences prefs;
                int portTestResult;
    ...
    void run()
    {
        qInfo() << "DEBUG run: taskId" << m_TaskId << "starting, m_AboutToQuit:" << m_AboutToQuit;
        ...
        qInfo() << "DEBUG: about to call fetchServerInfo()";
        QString serverInfo = fetchServerInfo(http);
        qInfo() << "DEBUG: fetchServerInfo returned, serverInfo length:" << serverInfo.length();
        ...
        if (serverInfo.isEmpty()) {
            qInfo() << "DEBUG: serverInfo is empty, returning";
            return;
        }

        qInfo() << "DEBUG: about to construct NvComputer";
        ...
        qInfo() << "DEBUG: NvComputer constructed, uuid:" << newComputer->uuid << "name:" << newComputer->name;

        // Apply caller-provided custom name (e.g. from backend login response)
        // before any update()/polling overwrites it. Setting hasCustomName=true
        // also makes NvComputer::update() preserve this name on subsequent polls.
        if (!m_CustomName.isEmpty()) {
            QWriteLocker nameLock(&newComputer->lock);
            newComputer->name = m_CustomName;
            newComputer->hasCustomName = true;
            qInfo() << "DEBUG: applied customName to newComputer:" << m_CustomName;
        }

        // V4: Apply caller-provided PIN (e.g. from backend login response)
        if (!m_Pin.isEmpty()) {
            QWriteLocker pinLock(&newComputer->lock);
            newComputer->pin = m_Pin;
            qInfo() << "DEBUG: applied device PIN to newComputer:" << m_Pin;
        }
...
int PendingAddTask::s_TaskCounter = 0;

void ComputerManager::addNewHost(NvAddress address, bool mdns, NvAddress mdnsIpv6Address, QString customName, QString pin)
{
    // Punt to a worker thread to avoid stalling the
    // UI while waiting for serverinfo query to complete
    PendingAddTask* addTask = new PendingAddTask(this, address, mdnsIpv6Address, mdns, customName, pin);
...
QString ComputerManager::generatePinString()
{
    ...
    // TODO:
    return ComputerManager::m_targetPin;
}

void ComputerManager::setLanFixedPinMode(bool enabled)
{
    m_LanFixedPinMode = enabled;
    qInfo() << "LAN fixed PIN mode:" << (enabled ? "enabled" : "disabled");
}

bool ComputerManager::getLanFixedPinMode()
{
    return m_LanFixedPinMode;
}

void ComputerManager::reloadLanFixedPinMode()
{
    QSettings settings("./conf.ini", QSettings::IniFormat);
    m_LanFixedPinMode = settings.value("lanFixedPin", false).toBool();
    qInfo() << "Reloaded LAN fixed PIN mode from conf.ini:" << m_LanFixedPinMode;
}

void ComputerManager::clearAllComputer()
{
    QVector<NvComputer*> newComputerList = getComputers();
    for (int i = 0; i < newComputerList.count(); i++)
    {
        deleteHost(newComputerList.at(i));
    }
}

void ComputerManager::setDeviceList(QVariantList devices)
{
    m_DeviceList = devices;
    qInfo() << "Device list saved, total" << devices.size() << "devices";
    for (int i = 0; i < devices.size(); i++) {
        QVariantMap device = devices[i].toMap();
        qInfo() << "Device" << i << ":" << device.value("device_name").toString();
    }
}

QVariantList ComputerManager::getDeviceList()
{
    return m_DeviceList;
}

QVariantList ComputerManager::fetchOnlineDevices(QString serverIp)
{
    QVariantList devices;

    QUrl url(QString("http://%1:8082/api/stream/v1/devices/online").arg(serverIp));
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        if (doc.isObject()) {
            ...
            if (obj.value("code").toInt() == 0) {
                QJsonArray arr = obj.value("data").toObject().value("devices").toArray();
                for (const QJsonValue& val : arr) {
                    QJsonObject dev = val.toObject();
                    QVariantMap item;
                    item["deviceId"] = dev.value("deviceId");
                    item["name"] = dev.value("name");
                    item["ip"] = dev.value("ip");
                    item["status"] = dev.value("status");
                    devices.append(item);
                }
            }
        }
    } else {
        qWarning() << "fetchOnlineDevices failed:" << reply->errorString();
    }
...
QVariantMap ComputerManager::queryDeviceStatus(QString serverIp, QString deviceId, QString devicePassword)
{
    QVariantMap result;

    // 添加 device_password 作为 Query 参数
    QUrl url(QString("http://%1:8082/api/stream/v1/devices/%2/status?device_password=%3")
                 .arg(serverIp)
                 .arg(deviceId)
                 .arg(devicePassword));
    QNetworkRequest request(url);
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        if (doc.isObject()) {
            QJsonObject obj = doc.object();
            result["code"] = obj.value("code").toInt();
            result["msg"] = obj.value("msg").toString();
            if (obj.value("code").toInt() == 0) {
                QJsonObject dev = obj.value("data").toObject();
                result["deviceId"] = dev.value("deviceId");
                result["name"] = dev.value("name");
                result["ip"] = dev.value("ip");
                result["port"] = dev.value("port");
                result["pin"] = dev.value("pin");
                result["status"] = dev.value("status");
            }
        }
    } else {
        qWarning() << "queryDeviceStatus failed:" << reply->errorString();
        result["msg"] = reply->errorString();
    }
