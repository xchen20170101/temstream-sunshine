...
public:
    ...
    Q_INVOKABLE void addNewHostManually(QString address, QString customName = QString(), QString pin = QString());

    void addNewHost(NvAddress address, bool mdns, NvAddress mdnsIpv6Address = NvAddress(), QString customName = QString(), QString pin = QString());

    QString generatePinString();

    void pairHost(NvComputer* computer, QString pin, QString password = "");
    ...
    Q_INVOKABLE void clearAllComputer();
    Q_INVOKABLE void setDeviceList(QVariantList devices);
    Q_INVOKABLE QVariantList getDeviceList();
    // V4: 后端设备发现 API
    Q_INVOKABLE QVariantList fetchOnlineDevices(QString serverIp);
    Q_INVOKABLE QVariantMap queryDeviceStatus(QString serverIp, QString deviceId, QString devicePassword);

    // 纯局域网固定 PIN 模式
    Q_INVOKABLE void setLanFixedPinMode(bool enabled);
    Q_INVOKABLE bool getLanFixedPinMode();
    Q_INVOKABLE void reloadLanFixedPinMode();

    // 局域网模式 Sunshine 服务端IP
    Q_INVOKABLE QString getLanServerIp();
    Q_INVOKABLE void setLanServerIp(QString ip);

    // 局域网模式密码验证
    Q_INVOKABLE QVariantMap verifyLanPassword(QString serverIp, QString password);
    Q_INVOKABLE QString getLanPassword();
...
private:
    ...
    QString m_targetPin;
    QVariantList m_DeviceList;  // 存储设备列表
    bool m_LanFixedPinMode;  // 纯局域网固定 PIN 模式
};
