...
NvHTTP::getServerInfo(NvLogLevel logLevel, bool fastFail)
{
    ...
    //qInfo() << "DEBUG getServerInfo: m_ServerCert.isNull():" << m_ServerCert.isNull() << "httpsPort():" << httpsPort();
    ...
    if (!m_ServerCert.isNull() && httpsPort() != 0)
    {
        try
        {
            ...
            serverInfo = openConnectionToString(m_BaseUrlHttps,
                                                ...
                                                logLevel);
            //qInfo() << "DEBUG getServerInfo: HTTPS response length:" << serverInfo.length();
    ...
    else
    {
        ...
        //qInfo() << "DEBUG getServerInfo: using HTTP, baseUrl:" << m_BaseUrlHttp.toString();
        serverInfo = openConnectionToString(m_BaseUrlHttp,
                                            "serverinfo",
                                            nullptr,
                                            fastFail ? FAST_FAIL_TIMEOUT_MS : REQUEST_TIMEOUT_MS,
                                            logLevel);
        //qInfo() << "DEBUG getServerInfo: HTTP response length:" << serverInfo.length();
        verifyResponseStatus(serverInfo);
        //qInfo() << "DEBUG getServerInfo: verifyResponseStatus passed";
...
NvHTTP::verifyResponseStatus(QString xml)
{
    ...
    while (xmlReader.readNextStartElement())
    {
        if (xmlReader.name() == QString("root"))
        {
            ...
            //qInfo() << "DEBUG verifyResponseStatus: found root, status_code:" << statusCode;
            if (statusCode == 200)
            {
                // Successful
                return;
    ...
    //qWarning() << "DEBUG verifyResponseStatus: no root element found, atEnd:" << xmlReader.atEnd() << "hasError:" << xmlReader.hasError() << "errorString:" << xmlReader.errorString();

    if (xmlReader.atEnd() && !xmlReader.hasError()) {
        qWarning() << "verifyResponseStatus: XML is empty or no root element found";
                       ...
                       NvLogLevel logLevel)
{
    ...
    //qInfo() << "DEBUG openConnection: loop finished, reply->error():" << reply->error() << "isFinished:" << reply->isFinished();
...
// Verify LAN mode password against Sunshine server
QVariantMap NvHTTP::verifyLanPassword(QString serverIp, QString password)
{
    QVariantMap result;
    ...
    baseUrl.setHost(serverIp);
    baseUrl.setPort(47984);  // Sunshine HTTP port

    // Build URL without query parameters
    QUrl url(baseUrl);
    url.setPath("/api/verify_lan_password");

    QNetworkRequest request(url);
    request.setHeader(QNetworkRequest::KnownHeaders::ContentTypeHeader, "application/json");

    // Build JSON body with password
    QJsonObject json;
    ...
    if (timer.isActive()) {
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (doc.isObject()) {
                ...
                // Sunshine returns {"status": true} on success
                if (obj["status"].toBool()) {
                    result["success"] = true;
                    qInfo() << "LAN password verified successfully";
                } else {
                    result["msg"] = obj["error"].toString("Invalid password");
                    qWarning() << "LAN password verification failed:" << result["msg"].toString();
                }
            } else {
                result["msg"] = "Invalid server response format";
            }
        } else {
            // Network error
            ...
            if (statusCode == 401) {
                ...
                result["msg"] = QString("Connection failed: %1").arg(reply->errorString());
            }
            qWarning() << "LAN password verify request failed:" << reply->error() << reply->errorString();
        }
    } else {
        // Timeout
        reply->abort();
        result["msg"] = "Connection timeout, please check server address";
        qWarning() << "LAN password verify request timed out";
    }
