...
QByteArray
NvPairingManager::saltPin(const QByteArray& salt, QString pin, QString password)
{
    QByteArray result;
    result.append(salt);
    result.append(pin.toLatin1());
    if (!password.isEmpty()) {
        result.append(password.toLatin1());
    }
    return result;
}
...
//     // 在这里直接返回 PairState::PAIRED，跳过 PIN 验证步骤
//     PairState state = PairState::PAIRED;
//     // 如何设置状态
//     m_Http.openConnectionToString(m_Http.m_BaseUrlHttp, "pair", nullptr, REQUEST_TIMEOUT_MS);
...
NvPairingManager::pair(QString appVersion, QString pin, QSslCertificate& serverCert, QString password)
{
    ...
    QByteArray saltedPin = saltPin(salt, pin, password);
